...
async def parsecsv(file:UploadFile = File("test.csv")):
    ...
    return response
def retrcsv(files):
    return FastAPI_CSV(files)
@app.post("/direct_csv")
async def parsecsvdirect(csv_file:UploadFile = File("test.csv")):
    ...
    parsed = json.loads(result)
    response = StreamingResponse(io.StringIO(test_data.to_json("fatapi_predictions.json")), media_type="text/json")
    return parsed 
